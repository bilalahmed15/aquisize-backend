...
// Helper function to generate Authorization header with user API key
...
// Fetch all campaigns
const fetchCampaigns = async (apiKey) => {

  console.log("I m poked"+apiKey)
...
app.get("/api/lead-classification", async (req, res) => {
 const { apiKey } = req.query; // Get API key from query parameters
 if (!apiKey) {
 ...
 try {
   const campaignsData = await fetchCampaigns(apiKey);
...
const fetchCampaignStatsWithThrottle = async (campaignId, apiKey, delayTime = 500) => {
 try {
   const response = await axios.get(
...
app.get("/api/running-campaigns", async (req, res) => {
 ...
 try {
   ...
   for (const campaign of campaignsData) {
     const stats = await fetchCampaignStatsWithThrottle(campaign.id, apiKey);
     if (stats) {
